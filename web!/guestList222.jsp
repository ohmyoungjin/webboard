...
<jsp:useBean id ="sql2" class = "net.tis.sql.GuestSQL"/>
<jsp:useBean id ="dto2" class = "net.tis.sql.GuestDTO"/>
</head>
<body>
...
<%
	ArrayList<GuestDTO> SK=sql2.dbSelect();
...
%>   
   <tr  onmouseover="style.backgroundColor='rgb(0,200,200)'" 
     ...
     <td> <%= dto2.getSabun() %> </td> 
     <td> <%= dto2.getName() %> </td> 
     <td> <a href="guestDetail.jsp?idx=<%=dto.getTitle()%>"> <%= dto.getTitle() %> </a> </td> 
     <td>  </td> 
     <td>  </td>
     <td align="center">
      <input type="button" onclick="location.href='guestDelete.jsp?idx=<%=dto.getSabun()%>' " value="삭제">
     </td>    
   </tr>
<% } %>  
